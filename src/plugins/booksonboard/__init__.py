...
class Catalog(BookStore):
    ...
    def operate(self):
        ...
        if not self.w:
            ...
            self.crumbs=[]
            self.openUrl(QtCore.QUrl('http://booksonboard.com/xml/catalog.atom'))
    ...
    def openUrl(self, url):
        ...
        if not url.startswith('http'):
            url=urlparse.urljoin('http://booksonboard.com/xml/',url)        
    ...
    def parseBranch(self):
        ...
        t1 = time.time()
        # FIXME: why are these two not giving the same output???
        data = parse(unicode(self.w.store_web.page().mainFrame().toHtml()).encode('utf-8'))
        data = parse(urllib.urlopen(url).read())
        print data.feed
