        ...
        if query['title'] is None and \
           ...
           query['isbn'] is None:
            QtGui.QMessageBox.warning(self,
                                     ...
                                     self.tr(u'You need to select at least '
                                              'one field to search'))
            return
...
"""
                ...
                self.candidates.page().mainFrame().setHtml(u"<h3>%s</h3>" %
                     unicode(self.tr(
                             u'No matches found for the selected criteria')))
...
class BookEditor(QtGui.QWidget):
    ...
    def is_saved(self, parent=None):
        if self.unsaved:
            ...
            msgBox.setInformativeText(self.tr('Do you want to save '
                                              'your changes?'))
    ...
    def on_add_file_clicked(self):
        file_name = unicode(QtGui.QFileDialog.getOpenFileName(self,
                                                         self.tr(u'Add File')))
        if file_name and not \
           self.fileList.findItems(file_name, QtCore.Qt.MatchExactly):
    ...
    def on_actionfindBook_triggered(self, checked=None):
        ...
        if datos == -1:
            QtGui.QMessageBox.critical(self,
                       self.trUtf8("Error"),
                       self.trUtf8("Please check the ISBN, "
                                   "it seems wrong."),
            ...
            QtGui.QMessageBox.critical(self,
                   self.trUtf8("Error"),
                   self.trUtf8("The ISBN seems to be valid, but no book was "
